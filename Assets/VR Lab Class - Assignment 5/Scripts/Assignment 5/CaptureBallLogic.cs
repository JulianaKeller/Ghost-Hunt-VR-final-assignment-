...
public class CaptureBallLogic : NetworkBehaviour
{
    ...
    IEnumerator AnimateGhostCapture(GameObject ghost)
    {
        ...
        // Play capture sound for all clients
        if (IsServer)
        {
            PlayCaptureSoundClientRpc();
        }
        ...
        {
            RequestDespawnCaptureBallServerRpc();
        }
    }
    ...
    IEnumerator DespawnCaptureBallAfterDelay()
    {
        ...
        Destroy(gameObject); // Ensure the object is also destroyed locally
    }

    #region RPCs

    [ServerRpc(RequireOwnership = false)]
    void RequestDespawnCaptureBallServerRpc()
    {
        StartCoroutine(DespawnCaptureBallAfterDelay());
    }
    ...
    [ClientRpc]
    void PlayCaptureSoundClientRpc()
    {
